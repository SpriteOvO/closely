...
pub enum SourceConfig {
    ...
    #[serde(rename = "GitHub.issue_pr")]
    GitHubIssuePr(Accessor<github::source::ConfigParams>),
    #[serde(rename = "RSS")]
...
impl Validator for SourceConfig {
    fn validate(&self) -> anyhow::Result<()> {
        match self {
            ...
            Self::GitHubIssuePr(p) => p.validate(),
...
impl fmt::Display for SourceConfig {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::GitHubIssuePr(p) => write!(f, "{p}"),
...
pub fn sourcer(platform: &Accessor<SourceConfig>) -> Sourcer {
    match &**platform {
        ...
        SourceConfig::GitHubIssuePr(p) => {
            Sourcer::new_fetcher(github::source::Fetcher::new(p.clone()))
        }
